...
import { CharacterCard,LoadingCard } from "../03-examples";
...
export const Layout =  () => {
    ...
    const {image, name, status, location, gender , species} = !!data && data;
  ...
  return (
   ...
   <div className="flex w-full gap-3 items-center max-w-[600px] justify-center">
       ...
       className='shadow-white shadow-xl  z-[9999] transition ease-in-out group hover:scale-105 hover:bg-slate-800 bg-slate-100 flex items-center justify-between  rounded-3xl p-4 '>
    ...
    {isLoading? <LoadingCard/> : <CharacterCard image={image} name={name}  status={status} location={location.name} gender={gender} species={species} />}
