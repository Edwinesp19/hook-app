...
export default function TodoApp() {
    ...
    <>
     ...
     <div className="grid grid-cols-1 lg:grid-cols-[1fr,1fr] h-screen md:h-full md:p-4 md:pb-6 w-full gap-4 max-w-[1300px]">
       <TodoClock/>
            <div className="  padding: env(safe-area-inset-top, 20px) env(safe-area-inset-right, 20px) env(
      safe-area-inset-bottom,
      20px
    ) env(safe-area-inset-left, 20px); animate-fade-up md:animate-fade-left animate-delay-100 animate-once animate-ease-in-out animate-alternate h-screen md:h-[95vh] w-full grid grid-rows-[100px,auto,50px] md:grid-rows-[120px,auto,50px] gap-2 p-2 md:p-6 md:border-[1px] md:border-white/20 bg-black/30 backdrop-blur-lg md:rounded-3xl">
